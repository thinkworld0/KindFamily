...
import net.thinkworld.kindfamily.util.DemoResHelper;
...
public class VerifyActivity extends Activity implements View.OnClickListener {
    ...
    private void initListener() {
        ...
        etCode.addTextChangedListener(new TextWatcher() {
            ...
            @Override
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                tvVerify.setEnabled(etCode.getText() != null && etCode.getText().length() >= 4 && etPhone.getText() != null && etPhone.getText().length() > 5);
            }
    ...
    private void initPrivacyDialog() {
        if (!DemoSpHelper.getInstance().isPrivacyGranted()) {
            PrivacyDialog privacyDialog = new PrivacyDialog(VerifyActivity.this, new OnDialogListener() {
                @Override
                public void onAgree() {
                    uploadResult(true);
